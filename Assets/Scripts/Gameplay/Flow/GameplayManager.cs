...
namespace XeptGame.Gameplay
{
    ...
    /// 装配器等场景侧经 EventBus 订阅 <see cref="GameplayStateChangedEvent"/>，**不持本类实例**。
    /// </summary>
    public static class GameplayManager
    {
        /// <summary>游戏流程状态机（单层：Boot → GameplayLoad → LevelLoad → Playing ⇄ Paused）。</summary>
        public static Fsm GameplayFSM { get; private set; }

        /// <summary>游戏流程上下文（会话数据唯一载体；Gameplay 域总线生命周期归本类）。</summary>
        public static GameplayContext Context { get; private set; }
        ...
        /// </summary>
        public static void Start(GameplayContext context)
        {
            if (GameplayFSM != null)
            {
                return;
            ...
            GameplayFSM = new Fsm(context);
            GameplayFSM.StateChanged += s => Log.Info($"[GameplayFSM] State change from {s.From?.ToString() ?? "启动"} to {s.To}");
            GameplayFSM.StateChanged += OnStateChanged;

            // 跨域桥接：Gameplay 域错误事件 → App 域（AppManager 订阅不变；Gameplay 域总线独立生命周期）
            context.EventBus.Subscribe<GameplayFlowErrorEvent>(e => context.AppEventBus.Publish(e));

            GameplayFSM.RequestChange<BootState>(_lifecycleCts.Token);
        ...
        {
            Context?.EventBus.Publish(new GameplayStateChangedEvent(args.To, args.From));
        }
    }
}
