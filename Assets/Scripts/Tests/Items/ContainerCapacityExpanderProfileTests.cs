...
{
    /// <summary>
    /// 容器扩容侧配置资产测试（SlotStore_Design.md §6）：
    /// <see cref="ContainerCapacityExpanderProfile"/> 扩容格数（≥1 归一）与扩容面经 <c>GetFacet</c> 的取用形状。
    /// 假 Definition 的 Facet 用反射注入（EquipTestBase 先例）。
    /// 注：背包侧基础格数原由 <c>InventoryProfile</c> 承担，现已由物品侧 <see cref="ContainerFacetProfile"/> 取代
    /// （Item_Instance_Design.md DP2/T2），原 <c>InventoryProfileTests</c> 随之收敛为本文件。
    /// </summary>
    public class ContainerCapacityExpanderProfileTests
    {
