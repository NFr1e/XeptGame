...
{
    /// <summary>
    /// 物品背包面配置（由 <see cref="InventoryFacet"/> 持有的共享配置资产，Inventory_Facet_Design.md §3）：
    ...
    /// </summary>
    [CreateAssetMenu(
        menuName = XeptGameConsts.Editor.ItemInventoryFacetProfileMenuName,
        fileName = XeptGameConsts.Editor.ItemInventoryFacetProfileFileName,
        order = XeptGameConsts.Editor.ItemInventoryFacetProfileOrder)]
    public sealed class InventoryFacetProfile : ScriptableObject
    {
    ...
    /// <summary>
    /// 背包归类（<see cref="InventoryFacetProfile.category"/> 的取值域，Inventory_Facet_Design.md §3）：
